...
/**
 * We need to carefully pick the assignment of pins for various purposes.
...
/* uncomment the following line to configure the software for a 4 x 20 character display */
//#define LCD4x20
...
#define PRINTLINE1(a) printLine(0,(char *)a)
#define PRINTLINE2(a) printLine(1,(char *)a)

#ifdef LCD4x20
#define PRINTLINE3(a) printLine(2,(char *)a)
#define PRINTLINE4(a) printLine(3,(char *)a)
#endif
 ...
 */
char c[NUM_CHARS+1], b[NUM_CHARS+1];      
char printBuff[NUM_LINES][NUM_CHARS+1];  //mirrors what is showing on the two lines of the display
...
void setTXFilters(unsigned long freq){
  ...
  if (freq > 21000000L){  // the default filter has a 35 MHz cut-off
 ...
 * Note: In cw mode, it does not key the radio, it only puts it in tx mode
 */
...
void setup()
{
  ...
  /* if the button is pressed when the power is applied we will jump to the factory alignment */
  if (btnDown())
    factory_alignment();
}
