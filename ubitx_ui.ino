...
void printLine(char linenmbr, char *c) {
  if (strcmp(c, printBuff[(unsigned char)linenmbr])) {     // only refresh the display when there was a change
    lcd.setCursor(0, linenmbr);             // place the cursor at the beginning of the selected line
    lcd.print(c);
    strcpy(printBuff[(unsigned char)linenmbr], c);

    for (byte i = strlen(c); i < NUM_CHARS; i++) { // add white spaces until the end of the 16 characters line is reached
      lcd.print(' ');
    }
  }
}

// this builds up the second line of the display with frequency and mode
void updateDisplay() {
